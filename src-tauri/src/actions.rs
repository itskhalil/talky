...
// Session Action — toggle start/stop for meeting sessions
struct SessionAction;

impl ShortcutAction for SessionAction {
    fn start(&self, app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        use crate::managers::session::SessionManager;

        let sm = app.state::<Arc<SessionManager>>();

        if sm.get_active_session_id().is_some() {
            // End current session
            let sm = Arc::clone(&sm);
            let app = app.clone();
            tauri::async_runtime::spawn(async move {
                sm.stop_speaker_capture();
                if let Err(e) = sm.end_session() {
            ...
            let rm = app.state::<Arc<AudioRecordingManager>>();
            if let Err(e) = rm.start_session_recording() {
            ...
            match sm.start_session(None) {
                Ok(session) => {
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        crate::commands::session::spawn_speaker_capture(speaker_buf, shutdown);
                    }

                    // Spawn chunked transcription loop
                    let app = app.clone();
                    let session_id = session.id.clone();
                    tauri::async_runtime::spawn(async move {
                        run_session_transcription_loop(app, session_id).await;
                    });
                }
                Err(e) => {
    ...
    fn stop(&self, _app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        // Session toggle is handled entirely in start()
    }
}
...
pub static ACTION_MAP: Lazy<HashMap<String, Arc<dyn ShortcutAction>>> = Lazy::new(|| {
    ...
    map.insert(
        "session".to_string(),
        Arc::new(SessionAction) as Arc<dyn ShortcutAction>,
    );
    map.insert(
        "cancel".to_string(),
